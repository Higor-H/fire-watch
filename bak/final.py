...
def setup_rabbitmq():
    ...
    try:
        ...
        try:
            ...
            channel.queue_declare(queue='zona_teste', passive=True)
        except pika.exceptions.ChannelClosedByBroker as e:
            # reabre canal e cria a fila caso não exista
            if getattr(e, "reply_code", None) == 404:
                channel = connection.channel()
                channel.queue_declare(queue='zona_teste', durable=True)
            else:
...
def upload_data_to_server(data):
    ...
    try:
        ...
        channel.basic_publish(exchange='', routing_key='zona_teste', body=payload)
