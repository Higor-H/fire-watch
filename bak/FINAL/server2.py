...
def setup_rabbitmq():
    ...
    try:
        ...
        try:
            # verifica existência sem redefinir argumentos
            channel.queue_declare(queue='zona_teste2', passive=True)
        except pika.exceptions.ChannelClosedByBroker as e:
            # reabre canal e cria a fila caso não exista
            if getattr(e, "reply_code", None) == 404:
                channel = connection.channel()
                channel.queue_declare(queue='zona_teste2', durable=True)
            else:
                print("Erro ao verificar fila zona_teste2:", e)
                try:
                    channel = connection.channel()
                except Exception:
                    pass
...
def upload_data_to_server(data):
    ...
    try:
        ...
        channel.basic_publish(exchange='', routing_key='zona_teste2', body=payload)
        print("Dados enviados para fila zona_teste2:", payload)
    except Exception as e:
        print("Erro ao enviar para RabbitMQ:", e)
        # força nova tentativa na próxima chamada
        try:
